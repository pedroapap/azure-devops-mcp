...
Set `PERSONAL_ACCESS_TOKEN` before starting the server. Supported values:

- Base64-encoded `"<email>:<pat>"` (for example, `dXNlckBleGFtcGxlLmNvbTpteS1wYXQ=`)
- Raw PAT string (for example, `my-plain-pat-token`)
...
```json
{
  "servers": {
    "ado-onprem": {
      ...
      "env": {
        "PERSONAL_ACCESS_TOKEN": "base64-of-email-colon-pat-or-raw-pat",
