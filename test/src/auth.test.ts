...
describe("createAuthenticator", () => {
  ...
  describe("pat strategy", () => {
    it("should return PAT from PERSONAL_ACCESS_TOKEN environment variable", async () => {
      process.env.PERSONAL_ACCESS_TOKEN = "test-pat-value";
      const authenticate = createAuthenticator("pat");
      const token = await authenticate();
      expect(token).toBe("test-pat-value");
    });

    it("should throw when PERSONAL_ACCESS_TOKEN is not set", async () => {
      const authenticate = createAuthenticator("pat");
      await expect(authenticate()).rejects.toThrow("Environment variable 'PERSONAL_ACCESS_TOKEN' is not set or empty. Please set it with a valid base64-encoded Azure DevOps Personal Access Token.");
    });
  });
